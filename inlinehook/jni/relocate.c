...
static void relocateInstructionInThumb(uint32_t target_addr, int length, uint16_t *trampoline_instructions)
{
	uint16_t *orig_instructions = (uint16_t *) target_addr;
	int orig_pos;
	int trampoline_pos;
	uint32_t pc;
	uint32_t lr;
...
static void relocateInstructionInArm(uint32_t target_addr, int length, uint32_t *trampoline_instructions)
{
	uint32_t *orig_instructions = (uint32_t *) target_addr;
	uint32_t pc;
	uint32_t lr;
	int orig_pos;
	int trampoline_pos;
...
void relocateInstruction(uint32_t target_addr, void *trampoline_instructions, int length)
{
	if (target_addr & 1 == 1) {
		relocateInstructionInThumb(target_addr - 1, length, (uint16_t *) trampoline_instructions);
	}
	else {
		relocateInstructionInArm(target_addr, length, (uint32_t *) trampoline_instructions);
	}
}
